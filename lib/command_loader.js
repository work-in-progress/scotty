  ...
  */
  exports.CommandLoader = (function() {
    function CommandLoader() {
      this.getResourceUsage = __bind(this.getResourceUsage, this);
    ...
    */
    ...
    CommandLoader.prototype.getActionFn = function(resourceName, actionName, cb) {
      ...
      if (action) {
        res = action.resource[actionName];
      }
      if (!(action && res)) {
    ...
    CommandLoader.prototype.getResourceUsage = function(resourceName, cb) {
      var cmd, res;
      cmd = this.commands[resourceName];
      if (!cmd) {
        return cb(new Error("Resource " + resourceName + " not found."));
      }
      res = [];
      _.each(cmd.resource.usage, __bind(function(value) {
        var item, _i, _len;
